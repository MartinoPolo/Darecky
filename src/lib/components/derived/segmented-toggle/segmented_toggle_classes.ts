...
export const segmentedToggleVariants = tv({
	slots: {
		...
		root: 'h-(--size-control-md) gap-px overflow-visible rounded-btn bg-accent shadow-none',
		item: 'size-(--size-control-md) rounded-[calc(var(--radius-btn)-2px)] border-0 bg-transparent shadow-none focus-visible:!outline-offset-2 focus-visible:!outline-ring data-[state=on]:bg-card data-[state=on]:text-foreground data-[state=on]:outline-2 data-[state=on]:outline-solid data-[state=on]:outline-offset-[-2px] data-[state=on]:outline-ink',
	},
});
