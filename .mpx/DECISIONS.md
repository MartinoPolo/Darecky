...
Decided: 2026-08-12 (issue #255; supersedes issue #213's all-surface release UI placement only, not its authorization; visual hierarchy revised by "Wishlist control geometry and responsive continuity" below)
What: Mark received/unreceived remains available to managers in card, list, and compact browse views, but is visually secondary; Reserve is the primary action whenever reservation is permitted. Marking a gift received enables the received filter so its updated action and badge remain visible. Privileged reservation release lives in the gift detail/editor dialog: správci use the edit form, while the app-admin override remains reachable from the read-only detail. Fully reserved gifts omit redundant disabled reservation actions. Existing recipient privacy and role authorization remain unchanged.
Why: Completion must react visibly from refreshed browse state without competing with the primary reservation workflow, while reservation status already communicates that a gift is unavailable and privileged release belongs in deliberate gift-detail management rather than routine browsing.

### Gift card footer action alignment

What: The gift card's primary/More action group anchors to the right content edge regardless of role, received state, or whether Like is present. Reserve remains the primary action; received/unreceived is secondary. Each action keeps its intrinsic compact visual size and accessible hit area whether rendered alone or beside another action. Like remains a separate secondary control at the full card/item's top-right in every layout, never in a thumbnail or footer; store links and inline description controls remain with their content.
...
Rejected: Left-aligned Received or More-only footers; changing a control's visual size because a neighboring control appears or disappears.
...
### Wishlist toolbar: labeled desktop controls, compact mobile command bar (superseded)

Decided: 2026-08-27 (issue #246 follow-up) — **Revised 2026-09-02 for narrow-screen sticky use; mobile behavior superseded 2026-09-04 by "Mobile wishlist command surfaces" and desktop behavior superseded by issue #359 below.**
...
### Mobile wishlist command surfaces (toolbar placement superseded)

Decided: 2026-09-04 (issue #340; supersedes the 2026-09-02 mobile three-trigger and fixed-two-row behavior; Settings placement superseded by issue #359 below)
...
### Wishlist toolbar: one Display command and hero Settings

Decided: 2026-09-05 (issue #359; supersedes the separate desktop controls and toolbar Settings placement above)
What: On desktop and mobile, the layout switcher is followed immediately by one Display trigger for sorting, grouping, and filtering. Desktop keeps the parent menu open while adjacent Sort, Group, and Filter submenus cascade like a Windows context menu; mobile retains the existing sectioned bottom sheet. Selected states, all options, role/privacy gates, active-filter visibility, and persistence semantics are unchanged. Capability-gated Settings is a gear-only action in the hero and nowhere in the toolbar or hero More menu. Hero More retains sharing and manager actions; toolbar More retains its eligible reset, preview, unfollow, selection, reorder, and batch-add actions. Add gift remains primary. Reorder has no obsolete leading separator and uses its full label where space permits. The accepted sticky toolbar mask background, blur, fade, layering, and regression coverage are unchanged.
Why: One predictable Display entry point reduces toolbar density without hiding state inside an in-place desktop drilldown, while moving list configuration to the list hero separates settings from browsing controls.
Rejected: Separate desktop Sort/Group/Filter triggers; replacing the desktop root menu with category contents; Settings in either toolbar or More menu; redesigning the accepted sticky mask.

### Wishlist control geometry and responsive continuity

What: Controls with the same purpose use the same compact visual height and icon sizing across the hero, toolbar, gift cards, and responsive breakpoints. Reserve remains visually primary whenever permitted and received/unreceived remains an available but visually secondary manager workflow; their visible surfaces keep intrinsic sizes independent of neighboring controls, and recipient privacy is unchanged. Desktop list-view actions are compact and right-aligned, and list view remains horizontal on mobile rather than changing into cards. Edge spacing is measured from the VISIBLE control surface, not its accessible hit box or shadow: Like sits at the full card/item's top-right in every layout, never in a thumbnail or footer, with equal top and right inset; a bottom-right action has equal right and bottom inset. Toolbars provide enough vertical room for controls to have balanced vertical and right insets. Nested corner radius follows `max(0, outer radius - border/padding inset)`. Implementations distinguish accessible hit area, visible surface, and shadow clearance rather than forcing one box to serve all three. Same-page action sheets use one consistently rounded, horizontally inset surface treatment even when they contain different action groups.
Why: Stable visual geometry prevents role, state, breakpoint, and neighboring-action changes from making equivalent controls jump, stretch, or appear uneven while preserving accessible targets.
Rejected: Breakpoint-specific reskinning of the same control; cardifying list rows on mobile; measuring inset from invisible hit-area or shadow bounds; coupling visible control size to sibling presence; flush or differently shaped action sheets on the same page.

### Wishlist list images remain square and fill the complete row height

What: Every normal horizontal wishlist list row, including mobile rows, renders the `thumb` image in a 1:1 visible frame that reaches from the card's inner top edge to its inner bottom edge, including beside the action region. These are simultaneous constraints: when content or actions make the row taller, the image frame becomes equally wider rather than retaining an independent width cap. Description is truncated first and title second when density is constrained; price, quantity, the store-link entry, and every eligible action remain available. Focused geometry coverage must assert both full-height edge alignment and the 1:1 frame across representative visitor, manager, reservation, and no-image states.
Why: A fixed self-starting square leaves a detached card-background gap below the image as soon as the content column becomes taller. Calling that box a “square thumbnail” satisfies the crop aspect in isolation while violating the approved row composition.
Rejected: A fixed or clamped thumbnail width independent of row height; testing only width-equals-height while permitting whitespace below; stretching the frame to a non-square rectangle; deleting full-height assertions to accommodate a compact thumbnail; turning the normal mobile list row into a card stack. The explicit enlarged-text accessibility fallback may still stack the complete square image above content when a horizontal row cannot retain all essential controls.

### All mobile wishlist bottom sheets use one shared contract

What: Every mobile bottom sheet on the wishlist page uses the same component and style contract, not merely a vaguely similar rounded treatment. `WishlistBottomSheet` is authoritative for the outer safe-area insets, border treatment, and top rounding; `WishlistSheetHeader` is authoritative for the divider, padding, and close-control geometry; `WishlistSheetBody` is authoritative for the consistent scroll region and body padding; and menu-like rows use `WishlistSheetAction`, whose inner button surface is left-aligned and whose minimum-48 touch row, icon sizing, spacing, and text-only label-column indent are shared. These canonical components, rather than copied pixel constants in callers or documentation, define the geometry. The contract applies to every bottom-sheet caller from the hero, toolbar More and Display, gift card/list More, and selection-mode nested actions and bulk copy. Purpose-specific form controls inside a sheet may retain distinct form styling, but they remain inside the shared shell, header, and body. The left-navigation drawer and centered dialogs are different surface types and are not bottom sheets.
Why: A shared wrapper alone does not prevent headers, body rhythm, close controls, or action rows from drifting. One composed contract makes every actual wishlist bottom-sheet entry point visually and behaviorally interchangeable while preserving legitimate differences between menu actions and form fields.
Verification: Maintain an inventory of every actual trigger that can open a wishlist bottom sheet and exercise each reachable role/state path. Capture the opened results and compare computed header and row styles, plus screenshots, rather than checking only wrapper dimensions. Any reported totals and PASS summaries must be derived from the captured results at verification time; they must never be hardcoded.
Rejected: Per-caller rounded-sheet approximations; wrapper-only parity checks; duplicated geometry constants; text-only rows without the shared label-column alignment; treating the mobile navigation drawer or centered dialogs as members of this contract.
